...
def onmessage(update,bot:ObigramClient):
    ...
    if username in tl_admin_users or len(tl_admin_users)<=0:  # validate user
        ...
        reply_markup = inlineKeyboardMarkup(
            r1=[inlineKeyboardButton('⚙Contactar Soporte⚙', url='https://t.me/Krixt0')]
        )
    ...
    if '/start' in text:
        ...
        start_markup = inlineKeyboardMarkup(
            r1=[inlineKeyboardButton(text='🪬Github (KrixtO1997)🪬',url='https://github.com/KrixtO1997?tab=repositories'),
                inlineKeyboardButton(text='⚙️Soporte (Telegram)⚙️',url='https://t.me/Krixt0')])
    ...
    if '/sync_'  in text or '/rm_' in text: 
        text = str(text).replace('/sync_','/sync ').replace(' /rm_','/rm ')    
    if '/rm' in text:
        index = None
        range = None
        try:
